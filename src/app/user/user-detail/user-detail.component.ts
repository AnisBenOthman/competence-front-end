...
export class UserDetailComponent implements OnInit, AfterViewInit {
  ...
  getAffectationbyUser() {
    this.affectationService
      .getAffectationByUser(this.ar.snapshot.params['id'])
      .subscribe({
        next: (data) => {
          ...
          this.affectations.map((o) => {
            ...
            this.radarDataSet = [
              {
                data: this.dataSet,
                label: `Competence ${this.user.nom}`,
                borderWidth: 1,
              },
            ];
          });
  ...
  addAffectation(data: any) {
    ...
    this.dataSet = [];
    this.affectationService
      .addAffectation(this.ar.snapshot.params['id'], data.competence.id, {
        niveau: data.niveau,
      })
      .subscribe({
        next: (data: any) => {
  ...
  declare(data: any) {
    (this.message = data.message), (this.alert = data.alert);
    setTimeout(() => {
  ...
  updateAffectation(data: any) {
    if (data.length > 0) {
      ...
      for (let i = 0; i < data.length; i++) {
        this.affectationService
          .updateAffectationEmploye(data[i].id, { niveau: data[i].niveau })
          .subscribe({
            next: (d: any) => {
              const index = this.labels.findIndex(
                (x) => x.toLowerCase() == d.employe.competenceId.libelle
              );
